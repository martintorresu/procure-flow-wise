...
var get_process_default = defineTool2({
  ...
  description: "Devuelve el detalle de un proceso de compra (por id o por n\xFAmero de proceso) junto con sus hitos y comentarios recientes.",
  inputSchema: {
  ...
  handler: async ({ id, pdc_number }, ctx) => {
    ...
    const [{ data: milestones }, { data: comments }] = await Promise.all([
      supabase.from("purchase_milestones").select("id, milestone_type, planned_date, actual_date, status, deviation_days").eq("pdc_id", process.id).order("planned_date", { ascending: true }),
